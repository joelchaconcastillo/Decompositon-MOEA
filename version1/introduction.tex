...
%
However, quite complex benchmarks have been defined~\cite{huband2006review} and it is known that some function features 
provoke significant degradation on the performance of \MOEAS{}.
%
...
%
In some cases, this is done explicitly through density estimators, %TODO: Citar algun density estimator
whereas in other cases, such as in
decomposition-based methods this is done indirectly with the use of a diverse set of weights. %TODO: Citar MOEA/D
%
Since optimizing most objective space quality indicators implies attaining a well-spread set of solutions in the
objective space, not considering this kind of diversity at all would result in quite ineffective optimizers.
%
...
Differently, when looking at the best optimizers for other areas of optimization, such as the single-objective case,
it is clear that several state-of-the-art methods introduce mechanisms to vary the trend of the 
...
%
Moreover, several authors have shown that modern \MOEAS{} suffer some drawbacks related to stagnation and premature 
...
two different diversities should be taken into account, i.e. the one on the variable space and the 
one on the objective space, and the relation between them is problem-dependent.
%
Note that the aforementioned design principle is based on reducing the importance of diversity on variable space as 
generations evolve, so we maintain this decision and consequently increase the importance of diversity on objective space as
...
%
In order to show the validity of our hypothesis, this paper proposes 
\textit{the Variable Space Diversity MOEA based on Decomposition} (\VSDMOEAD{}).
%, which explicitly manage the diversity in the 
%variable space to properly balance from exploration towards intensification.
%
Particularly, \VSDMOEAD{} simplifies \MOEADDE{}~\cite{zhang2009performance} by deactivating the dynamical resource allocation
